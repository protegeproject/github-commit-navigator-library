...
/**
 * Default implementation of {@link AuthenticationManager} that provides GitHub authentication and
 * credential management services.
 *
 * <p>This implementation supports multiple authentication methods including Personal Access Tokens,
 * OAuth tokens, and username/password combinations. It handles both GitHub API authentication and
 * JGit credential provider creation for Git operations.
 *
...
public class AuthenticationManagerImpl implements AuthenticationManager {
   ...
   *
   * <p>This method supports the following authentication types:
   *
   * <ul>
   *   <li>Personal Access Token - Uses OAuth token authentication
   *   <li>OAuth Token - Uses OAuth token authentication
   *   <li>Username/Password - Uses password-based authentication
   * </ul>
   *
   * <p>If no configuration is provided, anonymous authentication is attempted for accessing public
   * repositories.
   *
   * @param config the authentication configuration, or null for anonymous access
   * @return an authenticated {@link GitHub} instance
   * @throws AuthenticationException if authentication fails, if the authentication type is not
   *     supported, or if SSH key authentication is attempted
   */
  @Override
    ...
    try {
      ...
      return switch (config.getType()) {
        ...
        case USERNAME_PASSWORD ->
            builder.withPassword(config.getUsername(), config.getPassword()).build();
        case SSH_KEY ->
            throw new AuthenticationException(
                "SSH key authentication not supported for GitHub API");
        default ->
            throw new AuthenticationException(
                "Unsupported authentication type: " + config.getType());
      };
    } catch (IOException e) {
   ...
   *
   * <p>This method converts the authentication configuration into a JGit-compatible credentials
   * provider that can be used for Git operations such as clone, fetch, and push.
   *
   * <p>Supported authentication types and their JGit mappings:
   *
   * <ul>
   *   <li>Personal Access Token - Uses token as username with empty password
   *   <li>OAuth Token - Uses token as username with empty password
   *   <li>Username/Password - Uses provided username and password
   * </ul>
   *
   * @param config the authentication configuration, or null for anonymous access
   * @return a {@link CredentialsProvider} for Git operations, or null for anonymous access
   * @throws AuthenticationException if the authentication type is not supported or if SSH key
   *     authentication is attempted
   */
  @Override
  public CredentialsProvider getCredentialsProvider(AuthenticationConfig config)
      throws AuthenticationException {
    if (config == null) {
      logger.debug(
          "No authentication config provided, returning null credentials provider for anonymous access");
      return null;
    }
    ...
    return switch (config.getType()) {
      case PERSONAL_ACCESS_TOKEN, OAUTH ->
  ...
  /**
   * Validates the authentication configuration by checking credential format and testing
   * connectivity.
   *
   * <p>This method performs two levels of validation:
   *
   * <ol>
   *   <li>Format validation - Ensures credentials are not null/empty and properly formatted
   *   <li>Connectivity validation - Tests actual authentication with GitHub API
   * </ol>
   *
   * <p>For anonymous access (null configuration), validation is skipped.
   *
   * @param config the authentication configuration to validate, or null for anonymous access
   * @throws AuthenticationException if the configuration is invalid, credentials are malformed, or
   *     if authentication with GitHub fails
   */
  @Override
  ...
  private void validateSshKeyPath(String sshKeyPath) throws AuthenticationException {
    if (Optional.ofNullable(sshKeyPath)
        .map(String::trim)
